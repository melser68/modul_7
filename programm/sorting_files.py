...
def move_files(rez_file, folder_move, dyrectory_current):
    for file_rez in rez_file:        
        shutil.move(Path(dyrectory_current, file_rez ), folder_move)

#Розпаковуємо архіви, якщо вони є, та видаляємо самі архіви після розпакування


def unpack_archive(dyrectory_current):
    try:
        folder_archive = dyrectory_current + '\\archive_sorted'
...
def report_create_folder(dyrectory_current):
    if len(suffix_img) > 0:
        ...
        create_folder(dyrectory_current, 'image_sorted')
        
        move_files(rez_img, Path(dyrectory_current,'image_sorted'), dyrectory_current)
    ...
    if len(suffix_archive) > 0:
        ...
        create_folder(dyrectory_current, 'archive_sorted')
        move_files(rez_archive, Path(dyrectory_current,'archive_sorted'), dyrectory_current)
        unpack_archive() 
    ...
    if len(suffix_video) > 0:
        ...
        create_folder(dyrectory_current, 'video_sorted')
        move_files(rez_video, Path(dyrectory_current,'video_sorted'), dyrectory_current)
    else:
        print('__________________________________________________________')
    ...
    if len(suffix_music) > 0:
        ...
        create_folder(dyrectory_current, 'music_sorted')
        move_files(rez_music, Path(dyrectory_current,'music_sorted'), dyrectory_current)
    else:
        print('__________________________________________________________')
    ...
    if len(suffix_document) > 0:
        ...
        create_folder(dyrectory_current, 'documents_sorted')
        move_files(rez_documents, Path(dyrectory_current,'documents_sorted'), dyrectory_current)
    else:
        print('__________________________________________________________')
    ...
    if len(suffix_other) > 0:
        ...
        create_folder(dyrectory_current, 'other_sorted')
        move_files(rez_other, Path(dyrectory_current,'other_sorted'), dyrectory_current)
    else:
        print('__________________________________________________________')
...
def __main__():
    ...
    try:
        dyrectory_current = input('Введіть повний шлях до папки аналізу. - ')
        analiz_files(dyrectory_current)
        create_list_suffix()
        report_create_folder(dyrectory_current)
    ...
    chois = input('Повторити з іншою папкою ? (yes/no) - ').lower()
    if chois == 'yes':
        __main__()
